...
def calc_non_table(x_non, col, section, spliter):
    ...
    cnt_negative = pd.Series(spliter.predict(x_non.loc[x_non["target"] == 0, [col]],
    ...
    table = reduce(
        ...
        [section, cnt_rec, cnt_positive, cnt_negative])
...
def calc_mis_table(x_mis, col, special):
    if x_mis.empty:
        table = pd.DataFrame({
            col: [special],
            "CntRec": [0.],
        ...
        cnt_positive = x_mis.loc[x_mis["target"] == 1, [col]].value_counts(sort=False).to_frame("CntPositive")
        cnt_negative = x_mis.loc[x_mis["target"] == 0, [col]].value_counts(sort=False).to_frame("CntNegative")
        cnt_rec = cnt_positive["CntPositive"].add(cnt_negative["CntNegative"]).to_frame("CntRec")
...
def calc_cat_table(x_non, x_mis, col):
    ...
    # num to cat
    group_list = pd.DataFrame({col: [[] for _ in range(len(break_list))]}).reindex_like(break_list)
    for key, val in encoder.ordinal_encoder.mapping[0]["mapping"].items():
        for idx in break_list.index:
            if encoder.mapping[col][val] in break_list.loc[idx, col]:
                group_list.loc[idx, col].append(key)

    non_table = calc_non_table(x_non, col, group_list, spliter)
    mis_table = calc_mis_table(x_mis, col, "missing")
    ...
    table = table.reset_index(drop=True).fillna(0.)
...
def calc_num_table(x_non, x_mis, col):
    ...
    mis_table = calc_mis_table(x_mis, col, -999999.0)
    table = pd.concat([non_table, mis_table.reindex(columns=non_table.columns)])
